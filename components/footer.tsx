      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 py-16 lg:py-24">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8">
          <div className="lg:col-span-1">
            <Link href="/" className="text-2xl font-serif italic text-background tracking-tight">osmosis esp.</Link>
          ...
          <div>
            ...
            <ul className="space-y-2.5">
              {["Barcelona", "Terrassa", "Sabadell", "Badalona", "Mataró", "Granollers"].map(c => (
                <li key={c}><Link href={`/osmosis-inversa/${c.toLowerCase()}/`} className="text-xs text-background/60 hover:text-background transition-colors">{c}</Link></li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-3">
              ...
              <li>
                <a href="mailto:hola@formaypunto.com" className="flex items-center gap-2 text-xs text-background/60 hover:text-background transition-colors">
                  <Mail className="w-3.5 h-3.5" /> hola@formaypunto.com
                </a>
              </li>
        ...
        <div className="border-t border-background/10 mt-16 pt-8 flex flex-col sm:flex-row justify-between items-center gap-4">
          <p className="text-[10px] text-background/30">© {new Date().getFullYear()} osmosis esp. Todos los derechos reservados.</p>
